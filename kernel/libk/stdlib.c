...
void* kmalloc(size_t size) {
  ...
  while (current) {
    if (size <= current->size) { // found suitable block, first fit algorithm
      ...
      if (remainder < sizeof(block_header_t)) { // no suitable next block, use all memory chunk
        if (current == head) {
          head = current->next;
        }
        else {
      ...
      block_header_t* next_free_block = (block_header_t*)((uint8*)current + total_size);
...
void kfree(void* ptr) {
  ...
  while (memory_block->next) { // forward coalescing 
    ...
    if (((uintptr_t)(memory_block) & ~(PAGE_SIZE - 1)) != ((uintptr_t)(forward_block) & ~(PAGE_SIZE - 1))) { // check if blocks are on same page
      break;
    }
  ...
  if (((uintptr_t)(memory_block) & ~(PAGE_SIZE - 1)) != ((uintptr_t)(current) & ~(PAGE_SIZE - 1))) { // check if blocks are on same page
      return;
  }
