...
pid_t handle_fork() {
  ...
  task_t *child =
      task_alloc(parent->tickets); // copy parents priority within scheduler
  ...
  copy_user_pagetable((pte_t *)PA_TO_KVA(parent->pgd),
                      (pte_t *)PA_TO_KVA(child->pgd));
  child->parent = parent;
...
int handle_exec(const char *path, char* const argv[]) {
  ...
  if (argv) {
    if (copy_from_user(kva_pgd, argv, user_argv, sizeof(user_argv)) < 0)
      return -1;
  }
  ...
  uint64 sp = USER_STACK;
  uint64 user_ptrs[MAX_ARGS];
...
int handle_pipe(int p[]) {
  ...
  if (copy_to_user((pte_t *)PA_TO_KVA(current_task->pgd), p, fds,
                   sizeof(int) * 2) < 0) {
...
int handle_open(const char *path, int flags) {
  ...
  int len = strncpy_from_user(kva_pgd, path, buf, MAX_PATH);
  if (len < 0)
    return -1;
...
int handle_getcwd(void *buffer, size_t len) {
  if (!buffer || len == 0)
    return -1;
  ...
  uint32 current_cluster = current_task->cwd_cluster;
  if (current_cluster == root_cluster) {
    if (current_cluster < 2)
      return -1;
    kbuf[--pos] = '/';
  } else {
    while (current_cluster != root_cluster) {
      uint32 parent;
      char name[32];

      if (find_parent_and_name(current_cluster, &parent, name) < 0)
        return -1;
  ...
  pte_t *kva_pgd = (pte_t *)PA_TO_KVA(current_task->pgd);
  if (copy_to_user(kva_pgd, buffer, &kbuf[pos], final_len) != 0)
    return -1;
